...
class ResultReportServiceTest {
  ...
  void getHistory_DelegatesWithParsedSort() {
    ...
    given(
            resultReportQueryRepository.findHistory(
                "member-uuid", null, 10, ResultReportSort.LATEST, 31L))
        .willReturn(response);

    resultReportService.getHistory("member-uuid", null, 10, "latest", 31L);

    then(resultReportQueryRepository)
        .should()
        .findHistory("member-uuid", null, 10, ResultReportSort.LATEST, 31L);
  }

  @Test
  @DisplayName("복합 커서를 해석해 학습 히스토리 조회를 위임한다")
  void getHistory_DecodesCursor() {
    ResultReportCursor cursor =
        new ResultReportCursor(ResultReportSort.LATEST, LocalDateTime.of(2026, 7, 21, 10, 0), 701L);
    ResultReportHistoryResponse response =
        ResultReportHistoryResponse.of(List.of(), 10, null, false);
    given(
            resultReportQueryRepository.findHistory(
                "member-uuid", cursor, 10, ResultReportSort.LATEST, null))
        .willReturn(response);

    resultReportService.getHistory("member-uuid", cursor.encode(), 10, "latest", null);

    then(resultReportQueryRepository)
        .should()
        .findHistory("member-uuid", cursor, 10, ResultReportSort.LATEST, null);
  }
  ...
  void getHistory_RejectsUnsupportedSort() {
    assertThatThrownBy(
            () -> resultReportService.getHistory("member-uuid", null, 10, "popular", null))
  ...
  @Test
  @DisplayName("형식이 잘못되거나 정렬 조건과 불일치하는 커서를 거절한다")
  void getHistory_RejectsInvalidCursor() {
    ResultReportCursor oldestCursor =
        new ResultReportCursor(ResultReportSort.OLDEST, LocalDateTime.of(2026, 7, 21, 10, 0), 701L);

    assertThatThrownBy(
            () -> resultReportService.getHistory("member-uuid", "invalid", 10, "latest", null))
    ...
    assertThatThrownBy(
            () ->
                resultReportService.getHistory(
                    "member-uuid", oldestCursor.encode(), 10, "latest", null))
        .isInstanceOfSatisfying(
            CustomException.class,
            exception ->
                org.assertj.core.api.Assertions.assertThat(exception.getErrorCode())
